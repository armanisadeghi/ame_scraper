    ...
    def process_item(self, item, spider):
        ...
        if not page_id:
            ...
            execute_sql(self.conn, """
                ...
                """, (item["url"], domain_id, current_time, 200, current_time))  # Assuming HTTP status 200 and the current timestamp
            page_id = execute_sql(self.conn, "SELECT last_insert_rowid()", fetch_one=True)[0]
        ...
        # Process images
        image_data = [(page_id[0], img['src'], img['alt']) for img in item["images"]]
        ...
        if "header_structure" in item:
            ...
            execute_sql(self.conn, "INSERT INTO heading_structures (result_id, data) VALUES (?, ?)", (page_id[0], header_structure_json))

        # Insert StructuredData data
        if "schema_org_data" in item:
            schema_org_json = json.dumps(item["schema_org_data"])
            execute_sql(self.conn, "INSERT INTO structured_data (result_id, schema_org_data) VALUES (?, ?)", (page_id[0], schema_org_json))

        # Insert SitemapContent data
        if "sitemap_contents" in item:
            for sitemap_content in item["sitemap_contents"]:
                execute_sql(self.conn, "INSERT INTO sitemap_contents (domain_id, content) VALUES (?, ?)", (domain_id[0], sitemap_content))

        # Insert SitemapURL data
        if "sitemap_links" in item:
            for sitemap_link in item["sitemap_links"]:
                execute_sql(self.conn, "INSERT INTO sitemap_urls (domain_id, url) VALUES (?, ?)", (domain_id[0], sitemap_link))
